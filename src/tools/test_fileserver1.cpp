...
bool Login(Connection::ptr conn, const char *buffer, const int sockfd);

bool CheckPerm(Connection::ptr conn, const char *buffer, const int sockfd);

bool ExecSQL(Connection::ptr conn, const char *buffer, const int sockfd);
...
void run(int sockfd)
{
	...
	if (ExecSQL(conn, strsendbuf, sockfd) == false)
	{
		close(sockfd);
	}
...
bool CheckPerm(Connection::ptr conn, const char *buffer, const int sockfd)
{
	SERVER_LOG_INFO(g_logger) << "---------- CheckPerm --------------";
...
bool Login(Connection::ptr conn,const char *buffer, const int sockfd)
{
	...
	if (stmt.execute() != 0)
	{
		SERVER_LOG_INFO(g_logger) << "stmt.execute() failed.\n"
								<< "sql: \n" << stmt.m_sql
