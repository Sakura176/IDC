...
namespace server
{
	...
	static server::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >
		::ptr g_mysql_dbs = server::Config::Lookup("mysql.dbs", 
	...
	/**
	 * @brief 静态全局函数，用于初始化mysql数据库
	 * 
	 * @param params 数据库连接参数
	 * @param timeout 数据库连接等待时长
	 * @return MYSQL* 数据库指针
	 */
							...
							const int& timeout)
	{
		...
		if (mysql == nullptr)
		{
			SERVER_LOG_ERROR(g_logger) << "mysql_init error";
			return nullptr;
		}
		...
		int port = server::GetParamValue(params, "port", 3306);
		std::string host = server::GetParamValue<std::string>(params, "host");
		std::string user = server::GetParamValue<std::string>(params, "user");
		std::string passwd = server::GetParamValue<std::string>(params, "passwd");
		std::string dbname = server::GetParamValue<std::string>(params, "dbname");

		if (mysql_real_connect(mysql, host.c_str(), user.c_str(), passwd.c_str(),
					dbname.c_str(), port, NULL, 0) == nullptr)
		{
			SERVER_LOG_ERROR(g_logger) << "mysql_real_connect(" << host
									   << ", " << port << ", " << dbname
	...
	static MYSQL_RES* my_mysql_query(MYSQL* mysql, const char* sql)
	{
		if(mysql == nullptr)
		{
			SERVER_LOG_ERROR(g_logger) << "mysql_query mysql is null";
			return nullptr;
		}

		if (sql == nullptr)
		{
			SERVER_LOG_ERROR(g_logger) << "mysql_query sql is null";
			return nullptr;
		}

		if(::mysql_query(mysql, sql))
		{
			SERVER_LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:"
									   << mysql_error(mysql);
			return nullptr;
		}
		...
		if(res == nullptr)
		{
			SERVER_LOG_ERROR(g_logger) << "mysql_store_result() error:"
									   << mysql_error(mysql);
		}
		return res;
	}
	...
	bool MySQL::connect()
	{
		...
		m_hasError = false;
		m_poolSize = server::GetParamValue(m_params, "pool", 5);
	...
	{
		m_cmd = server::StringUtil::Formatv(format, ap);
		int r = ::mysql_query(m_mysql.get(), m_cmd.c_str());
		if (r)
		{
			SERVER_LOG_ERROR(g_logger) << "cmd=" << cmd()
									   << ", error: " << getErrStr();
			m_hasError = true;
		}
		else
	...
	{
		return MySQL::ptr(this, server::nop<MySQL>);
	}
	...
	{
		// 初始化函数返回的MYSQL_STMT语句处理程序数据结构。对于多次执行的语句，预处理执行是一种有效的方式
		...
		{
			SERVER_LOG_ERROR(g_logger) << "stmt=" << stmt
...
#define BIND_COPY_LEN(ptr, size)                                \
	...
	{
		return bindString(idx, server::Time2Str(value));
	}
	...
	{
		return server::TypeUtil::Atoi(m_cur[idx]);
	}
	...
	{
		return server::TypeUtil::Atof(m_cur[idx]);
	}
	...
	time_t MySQLRes::getTime(int idx) 
	{
		...
		return server::Str2Time(m_cur[idx]);
	}
	...
	{
		if(m_isFinished) {
			SERVER_LOG_ERROR(g_logger) << "transaction is finished, format=" << format;
			return -1;
		}
	...
	int MySQLTransaction::execute(const std::string& sql) {
		if(m_isFinished) 
		{
			SERVER_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
			return -1;
		}
	...
	MySQL::ptr MySQLManager::get(const std::string& name) 
	{
		...
		if(it != m_conns.end()) 
		{
			if(!it->second.empty()) 
			{
				...
				{
					SERVER_LOG_WARN(g_logger) << "reconnect " << name << " fail";
					return nullptr;
				}
			}
		}
	...
	int MySQLManager::execute(const std::string& name, const char* format, va_list ap) 
	{
		...
		if(!conn) 
		{
			SERVER_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
				<< ") fail, format=" << format;
			return -1;
		}
	...
	ISQLData::ptr MySQLManager::query(const std::string& name, const char* format, va_list ap) 
	{
		...
		if(!conn) 
		{
			SERVER_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
				<< ") fail, format=" << format;
			return nullptr;
		}
	...
	MySQLTransaction::ptr MySQLManager::openTransaction(const std::string& name, bool auto_commit) 
	{
		...
		if(!conn) 
		{
			SERVER_LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name
				<< ") fail";
			return nullptr;
		}
