...
struct st_arg
{
	...
	char	pname[31];			// 进程名，建议使用"ftpgetfiles_后缀"的方式
} starg;
...
void _help()
{
	...
	printf("<listfilename>/project/tmp/idc/ftpgetfiles_surfdata.list</listfilename> 下载前列出服务器文件名的文件。\n");
	printf("<ptype>1</ptype> 文件下载成功后, 远程服务器文件的处理方式: 1-什么也不做; 2-删除; 3-备份, 如果为3, 还要指定备份的目录。\n");
	printf("<remotepathbak>/project/tmp/bak/surfdata</remotepathbak> 文件下载成功后, 服务器文件的备份目录, 此参数只有当ptype=3时才有效。\n");
	printf("<okfilename>/project/tmp/local/ftpgetfiles_surfdata.xml</okfilename> 已下载成功文件名清单, 此参数只有当ptype=1时才有效。\n");
