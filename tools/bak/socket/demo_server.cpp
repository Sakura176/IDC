 ...
 * @brief 用于演示银行APP的多进程socket通讯服务端，增加心跳报文
 * 应用经验：
...
{
    if (argc != 4) {
        printf("Using:./demo_server port logfile timeout\nExample:./demo_server 5005 /project/logfile/tcp_server.log 35\n\n"); 
        return -1;
    }
    ...
    while (true)
    {
        ...
        while (true) {
            ...
            // 接收客户端的请求报文
            if ( tcpServer.Read(recvbuffer, atoi(argv[3])) == false) return -1;
